    ...
    <main className="max-w-[1200px] m-auto">
      ...
      <div className="flex px-8 mt-10">
        <div className="hidden sm:block w-1/4">
          ...
          <div className="my-2">
            <Link href="/" className="font-light bg-gray-700 rounded-lg px-6 py-2">Recent</Link>
          </div>
          <div className="my-2">
            <Link href="/browse" className="font-light rounded-lg px-6 py-2 hover:underline">Browse</Link>
          </div>
          <About />
        </div>

        <div className="w-3/4 m-auto">
          <h2 className="font-light text-2xl">{getPageName()}</h2>
          <ul role="list" className="divide-y divide-white">
            {mostRecentEntries.map((entry: Entry) => (
              <li
                key={entry.id}
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-x-2 text-xs leading-5 text-white-500">
                    <p>
                      <Link
                        href={{
                          pathname: `/${encodeURI(entry.podcastTitle || "")}`,
                          query: {
                            search: 'search'
                          }
                        }}
